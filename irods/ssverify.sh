...
find "$localpath" -type f |  while read filename; do
    ...
    adler=`$DIR/adler32 "$filename"`
    size=`/usr/bin/stat -c%s "$filename"`

#    test "$size" -eq 0 || echo "$adler:$size:$irodsfilepath" | ssh -o StrictHostKeyChecking=no -i $HOME/utils/ssverify.key irods@kali.uppmax.uu.se 2>/dev/null
     test "$size" -eq 0 || echo "$adler:$size:$irodsfilepath" | ssh johanhe@tintin ssh -o StrictHostKeyChecking=no -i /sw/apps/iRODS/utils/ssverify.key irods@kali.uppmax.uu.se #2>/dv/null
